...
function makeColumns(data) {
  ...
  keys = keys[0] ? [...cols.slice(0, 2).map(col => col.key), ...keys] : cols.map(col => col.key);
...
export async function downloadPNG(el, filename="chart.png") { 
  html2canvas(el, {windowWidth: el.scrollWidth, windowHeight: el.scrollHeight}).then(canvas => doDownload(canvas.toDataURL(), filename));
}